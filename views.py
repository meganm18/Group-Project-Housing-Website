...
def apartments_price(request):
	search_term = ''
	apartments = Apartment.objects.all()
	if 'price_search' in request.GET:
		search_term=request.GET['price_search']
		search_price= int('0'+search_term)
		apartments=apartments.filter(price_range=(0,search_price))
		return render(request, 'apartments.html', {'apartments': apartments, 'search_term': search_term})
